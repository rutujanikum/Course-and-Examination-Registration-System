        ...
        <div class="container">
            <%//Displaying flash data.
               try{
                   ...
                   <p class="alert alert-danger"><% out.print(request.getParameter("result"));%></p>
              <% }catch(Exception e){} %>
            <div class="row">
                <div class="col-sm-10">
       ...
       <%
            ...
            <h2>Pending courses</h2><hr>
             <% 
                        ...
                        { 
                            Course_reg coursereg=(Course_reg)obj2;
                            Courses course=(Courses)CourseCon.getCourseObject(coursereg.getCou_code().trim());
                            if(!(stud.getYear()==course.getYear() && stud.getTerm().equals(course.getTerm())))
                            {
                           
                    %>
                    <tr> 
                <td><%=j++%></td> 
                <td><input type="checkbox" name="courseP[]" value=" <%=course.getCourse_code()%>" <%=statusChk%> onchange="onBtn()"></td>
                <td><%=course.getCourse_code()%></td> 
                <td><%=course.getCourse_name()%></td>  
                <td><%=course.getCou_credit() %></td>  
                <td><%if(course.getCou_th_min()>0)out.print("Y"); else out.print("--"); %></td>  
        ...
        <input type="hidden" name="pageSource" value="editRegistration.jsp">
            </div>
        </div>
        </div> 
        <center>
